...
class AllocationState:
    ...
    def __init__(self, allocation_input: CanonicalAllocationInput):
    ...
    def try_assign(
        ...
        request = self._input.requests_by_key[request_key]
        section = self._input.logical_sections_by_id[linked_section_group_id]
    ...
    def validate_internal_consistency(self) -> tuple[StateConsistencyIssue, ...]:
        ...
        for key, assignment in sorted(self._assignments.items()):
            ...
            if assignment.student_id not in self._input.students_by_id:
                ...
                continue
            if assignment.request_key not in self._input.requests_by_key:
    ...
    def _assignment_rejection_reasons(
        ...
        request = self._input.requests_by_key.get(request_key)
        ...
        if request.student_id != student_id:
            ...
            if linked_section_group_id not in self._input.candidate_index.get(request_key, ()):
    ...
    def _require_request(self, request_key: str) -> None:
        if request_key not in self._input.requests_by_key:
