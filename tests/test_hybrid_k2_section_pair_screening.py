...
def test_formal_artifact_class_closure_and_run_a_solver_counters() -> None:
    ...
    assert counters["fixed_pair_feasibility_runs"] == 2
    assert counters["fixed_pair_guided_runs"] == 0
    assert counters["total_solver_invocations"] == 2
...
def test_formal_artifact_execution_counts_include_two_run_a_attempts() -> None:
    aggregate = read_artifact_json(FORMAL_ARTIFACT, "aggregate_summary.json")
    ...
    assert provenance["total_solver_invocations"] == 2
    assert provenance["fixed_pair_feasibility_runs"] == 2
